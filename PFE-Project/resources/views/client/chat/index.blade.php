...
@section('content')
<div class="mb-6">
    <h1 style="font-family:'Inter',sans-serif" class="text-2xl font-bold tracking-tight text-zinc-900 uppercase">Communications Hub</h1>
        ...
        <template x-if="!activeContact">
            <div class="flex-1 flex flex-col items-center justify-center p-8 bg-zinc-50/50">
                <div class="p-6 bg-white border border-zinc-200 shadow-sm text-center max-w-sm">
                    <i data-lucide="message-square" class="size-8 text-cyan-600 mx-auto mb-4 animate-bounce"></i>
                    <h3 class="text-[11px] font-mono font-bold uppercase tracking-wider text-zinc-800">Support Uplink</h3>
                    <p class="text-[10px] text-zinc-500 mt-2 leading-relaxed uppercase">Select your Coach or Admin to start real-time sync, ask questions, or upload forms/files.</p>
                </div>
            </div>
        </template>
                ...
                <div class="px-6 py-4 bg-white border-b border-zinc-200 flex items-center justify-between z-10">
                    <div class="flex items-center gap-x-3">
                        <div class="h-10 w-10 flex items-center justify-center font-mono text-xs font-bold text-white"
                             :class="activeContact.role === 'admin' ? 'bg-zinc-900 border border-zinc-800' : 'bg-cyan-700'">
    ...
    document.addEventListener('alpine:init', () => {
        Alpine.data('chatSystem', () => ({
            ...
            fetchContacts(silent = false) {
                ...
                fetch('{{ route("client.chat.contacts") }}')
                    .then(res => res.json())
                    .then(data => {
            ...
            fetchMessages(contactId, silent = false) {
                ...
                let url = '{{ route("client.chat.messages", ["contactId" => ":id"]) }}';
            ...
            sendMessage() {
                ...
                let url = '{{ route("client.chat.send", ["contactId" => ":id"]) }}';
